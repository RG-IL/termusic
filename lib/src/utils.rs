...
pub fn frecency_score(total_play_count: u64, last_played_at: Option<i64>, now: i64) -> f64 {
    ...
    let multiplier = match last_played_at {
        ...
        Some(lp) => {
            ...
            if elapsed < FrecencyTimeBucket::Hour.as_secs().cast_signed() {
                FrecencyTimeBucket::Hour.multiplier()
            } else if elapsed < FrecencyTimeBucket::Day.as_secs().cast_signed() {
                FrecencyTimeBucket::Day.multiplier()
            } else if elapsed < FrecencyTimeBucket::Week.as_secs().cast_signed() {
...
mod tests {
    ...
    #[test]
    #[allow(clippy::float_cmp, clippy::float_equality_without_abs)]
    fn frecency_score_basic() {
        let now = 1_700_000_000;

        // total_play_count=0 → score is 0 regardless of last_played_at
        assert_eq!(frecency_score(0, Some(now), now), 0.0);
        assert_eq!(frecency_score(0, None, now), 0.0);

        // 1 play, < 1 hour ago → rank 1 × 4.0 = 4.0
        assert_eq!(frecency_score(1, Some(now - 1_800), now), 4.0);

        // 1 play, < 1 day ago → rank 1 × 2.0 = 2.0
        assert_eq!(frecency_score(1, Some(now - 7_200), now), 2.0);

        // 1 play, < 1 week ago → rank 1 × 0.5 = 0.5
        assert_eq!(frecency_score(1, Some(now - 100_000), now), 0.5);

        // 1 play, ≥ 1 week ago → rank 1 × 0.25 = 0.25
        assert_eq!(frecency_score(1, Some(now - 700_000), now), 0.25);

        // 1 play, never played → rank 1 × 0.25 = 0.25
        assert_eq!(frecency_score(1, None, now), 0.25);

        // 3 plays, < 1 week ago → rank 3 × 0.5 = 1.5
        assert_eq!(frecency_score(3, Some(now - 100_000), now), 1.5);
    }
